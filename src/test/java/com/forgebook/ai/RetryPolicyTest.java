...
class RetryPolicyTest {
    ...
    void test6_retryAfterOverridesExponentialBackoff() {
        // DEFAULT maxDelay is 10s (raised from 5s in 1.0.4 so short retry-after
        // hints aren't clamped). retry-after SHOULD take precedence over the
        // exponential schedule BUT still be clamped at maxDelay (we never want
        // to sleep longer than the operator cap).
        //   retryAfter=3s < 10s maxDelay → literal 3s is used.
        //   retryAfter=15s > 10s maxDelay → clamped to 10s.
        RetryPolicy policy = RetryPolicy.DEFAULT;
        ...
        assertEquals(10_000L, policy.delay(0, Optional.of(over)).toMillis(),
